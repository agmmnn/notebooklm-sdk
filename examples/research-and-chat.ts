...
async function main() {
  ...
  try {
    ...
    const query = "What are the latest breakthroughs in solid-state batteries in 2026? Give me specific companies and dates.";
    ...
    if (result.status === "completed") {
      ...
      if (result.sources.length > 0) {
        ...
        const imported = await client.research.importSources(notebookId, result.taskId!, sourcesToImport);
        ...
        const chatQuestion = "Based strictly on the imported sources, what are the names of the companies working on solid-state batteries?";
